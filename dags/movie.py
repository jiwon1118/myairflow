...
with DAG(
    ...
    REQUIREMENTS = ["git+https://github.com/jiwon1118/movie.git@0.3.2"]
    ...
    def fn_merge_data(ds_nodash, base_path):
        from movie.api.call import fill_unique_ranking, save_merge_df
        import pandas as pd
        
        PATH= f"{base_path}/dt={ds_nodash}"
        df = pd.read_parquet(PATH)
        merge_df = fill_unique_ranking(df, ds_nodash)
        save_path = save_merge_df(merge_df, ds_nodash)
        
        print(ds_nodash, save_path)
